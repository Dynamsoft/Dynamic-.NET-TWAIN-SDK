    ...
    Private Sub picboxScan_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles picboxScan.Click
        If picboxScan.Enabled Then
            If (Me.cbxSource.SelectedIndex < 0) Then
                MessageBox.Show(Me, "There is no scaner detected! " & Constants.vbCrLf & "Please ensure that at least one (virtual) scanner is installed.", "Information")
            Else
...
#Region "Read Barcode"
    ...
    Private Sub picboxReadBarcode_Click(ByVal sender As Object, ByVal e As EventArgs) Handles picboxReadBarcode.Click
        ShowSelectedImageArea()
        Try
            dynamicDotNetTwain.MaxBarcodesToRead = Integer.Parse(tbxMaxBarcodeReads.Text)
        ...
        Dim aryResult As Dynamsoft.DotNet.TWAIN.Barcode.Result() : aryResult = dynamicDotNetTwain.ReadBarcode(dynamicDotNetTwain.CurrentImageIndexInBuffer, Integer.Parse(tbxLeft.Text), Integer.Parse(tbxTop.Text), _
            Integer.Parse(tbxRight.Text), Integer.Parse(tbxBottom.Text), DirectCast(cbxBarcodeFormat.SelectedValue, Dynamsoft.DotNet.TWAIN.Enums.Barcode.BarcodeFormat))

        If Not dynamicDotNetTwain.ErrorCode = Dynamsoft.DotNet.TWAIN.Enums.ErrorCode.Succeed Then
            MessageBox.Show(dynamicDotNetTwain.ErrorString, "Reading barcode error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Else
            Dim strResult As String : strResult = aryResult.Length & " total barcode found." & Constants.vbCrLf
            Dim i As Integer
            For i = 0 To aryResult.Length - 1
                strResult += "Result " & (i + 1).ToString() + Constants.vbCrLf & "  Barcode Format: " & aryResult(i).BarcodeFormat.ToString() & "    Barcode Text: " & aryResult(i).Text & Constants.vbCrLf
            Next i
